...
async function callOllama(imageBase64, mediaType, baseUrl, model) {
  let response
  try {
    response = await fetch(`${baseUrl.replace(/\/$/, '')}/api/chat`, {
      ...
      body: JSON.stringify({
        model,
        messages: [{ role: 'user', content: EXTRACTION_PROMPT, images: [imageBase64] }],
        stream: false,
        format: 'json',
      }),
    })
  ...
  return extractJsonItems(data.message?.content)
}
...
export const ollamaStrategy = {
  ...
  parse: (imageBase64, mediaType) => {
    ...
    return callOllama(imageBase64, mediaType, ollamaUrl || DEFAULT_URL, ollamaModel || DEFAULT_MODEL)
  },
}
